...
<html>
  <head>
	...
	<link href="css/videoNoteList.css" rel="stylesheet" type="text/css" />
 	...
 	<script type="text/javascript" src="js/videoNoteList.js"></script>
   ...
   <div id="max">
    ...
    <div class="show">
     <div id="videoShowList">
    	<form id="myform" action="VideoNoteServlet?op=delCheckedVideoNote" method="post">
    	<table width="1100px" style="margin-bottom: 10px;margin-left: -66.5px;">
    		...
    		<tr height="70px" id="menu">
    			...
    			<td width="100px" id="ck_td">
	    			<a id="delCKNotes" style="font-size: 16px;cursor:pointer" title="删除多个">删除多个</a>
    			</td>
    			<td id="quxiao" width="100px">
	    			<a style="font-size: 16px;cursor:pointer" title="取消删除">取消</a>
    			</td>
    			<td width="100px" id="clearAll_td">
			    	<a id="delAll" style="font-size: 16px;" href="javascript:delAllNotes();" title="清空收藏列表">清空记录</a>
    			</td>
    			<td>${user.nickname}的观看记录</td>
    		</tr>
 			</table>
    		<c:forEach items="${videoNoteList}" var="vn">
    		<ul class="movie_ul">
            	<li class="m_pic">
                    <input type="checkbox" class="note" name="note" title="选择《${vn.video.videoname }》" value="${vn.noteid }" onclick="checkBox();"/>
                    <a class="pic_a"  target="_blank">
                        <img src="upload/picture/${vn.video.picturepath }" width="175px" height="245px" title="${vn.video.title }" alt="${vn.video.videoname }"/>	
                    </a>
                    <input type="hidden" value="${vn.video.videoid }"/>
                </li>
                <li class="m_name">
                	<a href="VideoServlet?videoid=${vn.video.videoid }&op=playVideo" target="_blank">${vn.video.videoname }</a>
                </li>
                <li class="m_jieshao">
                	<span class="m_gray">导演:</span>${vn.video.director }
                </li>
                 <li class="m_jieshao" title="${vn.video.actor }">
                	  <span class="m_gray">主演:</span>${vn.video.actor }
                </li>
                 <li class="m_date">
                 	  <span class="m_time">${vn.notedate}</span>
                	  <span class="m_del"><a title="删除《${vn.video.videoname }》" href="javascript:delNote(${vn.noteid});">删除记录</a></span>
                </li>
            </ul>
    		</c:forEach>
    	</form>
    	<div id="open">
    		<p><a title="展开所有观看记录" href="javascript:open();">展开全部</a></p>
    	</div>
    	<div id="empty">
    		<p><a href="VideoServlet?op=getVideoFromTimeAndCount">暂时没有观看记录，最新最全的电影尽在青鸟视频！</a></p>
    	</div>
    	</div>
	   		<c:if test="${totalPages>1 }">
	           	...
	           	<c:if test="${pageIndex>1 }">
	            	<li><a href="VideoNoteServlet?op=showVideoNoteList&pageIndex=1">首页</a></li>
	            	<li><a href="VideoNoteServlet?op=showVideoNoteList&pageIndex=${pageIndex-1 }">上页</a></li>
	           	</c:if> 
	           	<c:if test="${pageIndex<=1 }">
	           		<li>首页</li>
	            	<li>上一页</li>
	           ...
	           <select id="SelectIndex" onchange="changeIndex();">
	           	...
	           	<c:if test="${pageIndex!=totalPages }">
	            	<li><a href="VideoNoteServlet?op=showVideoNoteList&pageIndex=${pageIndex+1 }">下页</a></li>
	            	<li><a href="VideoNoteServlet?op=showVideoNoteList&pageIndex=${totalPages }">尾页</a></li>
				</c:if>
				<c:if test="${pageIndex==totalPages }">
	           		<li>下页</li>
	            	<li>尾页</li>
