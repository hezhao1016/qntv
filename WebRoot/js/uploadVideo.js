...
function checkVideoPath(){
	var val = $("#videofile").val();
	var show = $("#videofile_pram");
	if(val == null || val == ""){
		show.html("✘ 您还没有选择视频文件");
		red(show);
		return false;
	}
	show.html("✔");
...
function checkPicturePath(){
	var val = $("#picturefile").val();
	var show = $("#picturefile_pram");
	if(val == null || val == ""){
		show.html("✘ 您还没有选择电影封面");
		red(show);
		return false;
	}
	show.html("✔");
...
$(function(){
	$("#myform").submit(function(){
		if(checkVideoName() & checkTitle() & checkDirector() & checkActor()
  			& checkSize() & checkPicturePath() & checkVideoPath() & checkDescribe()){
  				return true;
  			}else{
  				return false;
  			}
	 });
	$("#picturefile").blur(checkPicturePath).focus(function(){
		var show = $("#picturefile_pram");
		show.html("  请选择电影封面");
		orange(show);
	});
	$("#videofile").blur(checkVideoPath).focus(function(){
		var show = $("#picturefile_pram");
		show.html("  请选择视频文件");
		orange(show);
	});
